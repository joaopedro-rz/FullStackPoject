...
export function SearchForm() {
  const { setFilters, search, status, error, q, language, from, sortBy, pageSize, loadSavedArticles, mode } = useNews()
    ...
    <form className="search-form" onSubmit={onSubmit} noValidate>
      <div className="form-grid">
        ...
        <div className="form-actions" style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <button type="submit" className="btn btn-primary" disabled={status === 'loading' && mode === 'search'}>
            {status === 'loading' && mode === 'search' ? 'Buscando…' : 'Buscar Notícias'}
          </button>
          <button
            type="button"
            className="btn"
            disabled={status === 'loading' && mode === 'saved'}
            onClick={() => loadSavedArticles()}
          >
            {status === 'loading' && mode === 'saved' ? 'Carregando salvos…' : 'Ver artigos salvos'}
          </button>
        </div>
      </div>
